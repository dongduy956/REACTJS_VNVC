...
const Manager = () => {
    ...
    const namePage = namePages.PaymentMethod.name;
            ...
            render: (_, record) => {
                ...
                return editable ? (
                    <div className="flex justify-center">
                        <Typography.Link onClick={() => save(record)} className="mr-4">
                            Lưu
                        </Typography.Link>
    ...
    const save = async (record) => {
        ...
        try {
            ...
            if (record.id > 0) {
                const paymentMethods = (await paymentMethodService.getAllPaymentMethods()).data.filter(
                    (x) => x.name.toLowerCase().trim() !== record.name.toLowerCase().trim(),
                );
                if (paymentMethods.find((x) => x.name.toLowerCase().trim() === row.name.toLowerCase().trim()))
                    notification.warning({
                        message: 'Cảnh báo',
                        description: 'Trùng tên phương thức thanh toán.',
                        duration: 3,
                    });
                else {
                    const resultUpdate = await paymentMethodService.updatePaymentMethod(record.id, row);
    ...
    const fetchData = (params = {}) => {
        ...
        (async () => {
            ...
            if (params.searchText)
                res = await paymentMethodService.searchPaymentMethods(
                    params.searchText,
                    params.pagination.current,
                    params.pagination.pageSize,
                );
            else
                res = await paymentMethodService.getPaymentMethods(
                    params.pagination.current,
                    params.pagination.pageSize,
                );
            setLoading(false);
    ...
    return (
        <>
            <Head title={`${configTitle.paymentMethod}`} />

            <ManagerHeader
                pageName={namePage}
                setTable={handleSetData}
                titleImport="Nhập dữ liệu phương thức thanh toán"
                typeImport={typeImportExcel.paymentMethod}
            />
