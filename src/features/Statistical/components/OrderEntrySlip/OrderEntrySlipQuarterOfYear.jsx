...
const OrderEntrySlipQuarterYear = ({ form, setLoading }) => {
    const [data, setData] = useState([]);
    const [years, setYears] = useState([]);
    const fetchData = async (year) => {
        setLoading(true);
        if (year > 0) {
            const result = (await statisticalService.getOrderEntrySlipsQuarterOfYear(year)).data;
            setData(result.map((item) => ({ ...item, quarter: `Q${item.quarter}/${year}` })));
        }
        setLoading(false);
    };
    useEffect(() => {
...
export default OrderEntrySlipQuarterYear;
