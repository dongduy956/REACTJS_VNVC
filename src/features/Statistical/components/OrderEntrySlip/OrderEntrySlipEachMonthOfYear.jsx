...
const OrderEntrySlipEachMonthOfYear = ({ form, setLoading }) => {
    const [data, setData] = useState([]);
    const [years, setYears] = useState([]);
    const fetchData = async (year) => {
        setLoading(true);
        if (year > 0) {
            const result = (await statisticalService.getOrderEntrySlipsEachMonthOfYear(year)).data;
...
export default OrderEntrySlipEachMonthOfYear;
