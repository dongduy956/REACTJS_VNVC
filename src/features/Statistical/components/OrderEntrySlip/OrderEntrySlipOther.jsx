...
const OrderEntrySlipOther = ({ form, setLoading }) => {
    const { RangePicker } = DatePicker;
    const [data, setData] = useState([]);
    const fetchData = async (dateFrom, dateTo) => {
        setLoading(true);
        const result = (await statisticalService.getOrderEntrySlipsOther(dateFrom, dateTo)).data;
        setData(
            result.map((item) => ({
                ...item,
...
export default OrderEntrySlipOther;
