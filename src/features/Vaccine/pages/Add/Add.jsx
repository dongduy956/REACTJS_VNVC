...
const Add = () => {
    ...
    const [imageVaccine, setImageVaccine] = useState('');
    const props = {
        name: 'file',
        multiple: false,
    ...
    const fetchData = async (params) => {
        ...
        if (vaccines.find((x) => x.name.toLowerCase().trim() === params.name.trim().toLowerCase())) {
            notification.warning({
                message: 'Cảnh báo',
                description: 'Trùng tên vaccine.',
                duration: 3,
            });
        } else {
            const res = await vaccineService.insertVaccine(params);
            if (res.status === 500)
                notification.error({
                    message: 'Lỗi',
                    description: 'Có lỗi xảy ra khi thêm vaccine.',
                    duration: 3,
                });
    ...
    const onFinish = (params) => {
        params.image = imageVaccine;
        params.content = refEditor.current.value;
        fetchData(params);
    };
    return (
        <>
            <Head title={`${configTitle.add} ${configTitle.vaccine.toLowerCase()}`} />

            <Spin tip="Loading..." spinning={loading}>
                <Row>
                    <TitleAddUpdate>Thêm vaccine</TitleAddUpdate>
                    <Col span={24}>
                        <Form
                            name="wrap"
                            labelCol={{
