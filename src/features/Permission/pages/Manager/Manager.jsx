...
import { typeImportExcel, defaultPermissions, namePages, pageSizeOptions } from '~/constraints';
...
const Manager = () => {
    ...
    const namePage = namePages.Permission.name;
                ...
                return editable ? (
                    ...
                    <div className="flex justify-center">
                        {!Object.values(defaultPermissions).some(
                            (x) => x.toLowerCase().trim() === record.name.toLowerCase().trim(),
                        ) && (
                            <>
    ...
    const save = async (id) => {
        ...
        try {
            ...
            if (id > 0) {
                const resultUpdate = await permissionService.updatePermission(id, row);
    ...
    const fetchData = (params = {}) => {
        ...
        (async () => {
            ...
            if (params.searchText)
                res = await permissionService.searchPermissions(
                    params.searchText,
                    params.pagination.current,
                    params.pagination.pageSize,
                );
            else res = await permissionService.getPermissions(params.pagination.current, params.pagination.pageSize);
            setLoading(false);
    ...
    return (
        <>
            <Head title={`${configTitle.permission}`} />

            <ManagerHeader
                pageName={namePage}
                setTable={handleSetData}
                typeImport={typeImportExcel.permission}
                titleImport="Nhập dữ liệu chức vụ"
            />
