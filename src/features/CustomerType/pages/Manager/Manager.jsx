...
import { filterAges, namePages, pageSizeOptions, typeImportExcel } from '~/constraints';
...
const Manager = () => {
    ...
    const namePage = namePages.CustomerType.name;
    ...
    const columns = [
        ...
        },
        {
            ...
            onFilter: (value, record) =>
                value.length === 1 ? record.age > value[0] : record.age >= value[0] && record.age < value[1],
            editable: true,
        },
    ];
    ...
    const save = async (id) => {
        ...
        try {
            ...
            if (id > 0) {
                const resultUpdate = await customerTypeService.updateCustomerType(id, row);
    ...
    const fetchData = (params = {}) => {
        ...
        (async () => {
            ...
            if (params.searchText)
                res = await customerTypeService.searchCustomerTypes(
                    params.searchText,
                    params.pagination.current,
                    params.pagination.pageSize,
                );
            else
                res = await customerTypeService.getCustomerTypes(params.pagination.current, params.pagination.pageSize);
            setLoading(false);
    ...
    const handleDelete = (id) => {
        (async () => {
            setLoading(true);
            const res = await customerTypeService.deleteCustomerType(id);
            if (res.data)
                notification.success({
                    message: 'Thành công',
    ...
    return (
        <>
            <Head title={`${configTitle.customerType}`} />

            <ManagerHeader
                pageName={namePage}
                setTable={handleSetData}
                titleImport="Nhập dữ liệu loại khách hàng"
                typeImport={typeImportExcel.customerType}
            />
