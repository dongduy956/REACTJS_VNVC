...
const Manager = () => {
    ...
    const namePage = namePages.CustomerRank.name;
    ...
    const save = async (id) => {
        ...
        try {
            ...
            if (item) {
                const resultUpdate = await customerRankService.updateCustomerRank(id, { ...row, name: item.name });
    ...
    const fetchData = (params = {}) => {
        ...
        (async () => {
            ...
            if (params.searchText)
                res = await customerRankService.searchCustomerRanks(
                    params.searchText,
                    params.pagination.current,
                    params.pagination.pageSize,
                );
            else
                res = await customerRankService.getCustomerRanks(params.pagination.current, params.pagination.pageSize);
            setLoading(false);
    ...
    const handleDelete = (id) => {
        (async () => {
            setLoading(true);
            const res = await customerRankService.deleteCustomerRank(id);
            if (res.data)
                notification.success({
                    message: 'Thành công',
    ...
    return (
        <>
            <Head title={`${configTitle.customerRank}`} />

            <ManagerHeader
                pageName={namePage}
                setTable={handleSetData}
                titleImport="Nhập dữ liệu xếp loại khách hàng"
                typeImport={typeImportExcel.customerRank}
            />
